...
class RequestResolver implements Runnable {
    ...
    RequestResolver(Socket socket, Wardrobe wardrobe, Logger logger) {
        try {
        ...
        switch (message.getMessage()) {
            ...
            case "save":
                ...
                try {
                    ...
                    PleaseWaitMessage pleaseWaitMessage = new PleaseWaitMessage(out,
                            ...
                            2000);
                    WardrobeLoaderSaver.save(
                            wardrobe,(String)message.getArgument());
            ...
            case "import":
                ...
                try {
                    ...
                    PleaseWaitMessage pleaseWaitMessage = new PleaseWaitMessage(out,
                            "Ваш запрос в процессе обработки. Пожалуйста, подождите...",
                            2000);
                    WardrobeLoaderSaver.load(
                            wardrobe,
                            (String)message.getArgument()
                    );
                    pleaseWaitMessage.clear();
                    sendMessage("Загрузка успешна! В тюряге " + wardrobe.size() + " существ", endFlag);
                } catch (IOException e) {
                    sendEndMessage("На сервере произошла ошибка чтения/записи");
                } catch (WardrobeOverflowException e) {
            ...
            case "add":
                try {
                    ...
                    wardrobe.add(hat);
                    sendMessage(hat.getColor() + " добавлена в гардероб", endFlag);
                    return;
                } catch (WardrobeOverflowException e) {
        ...
        switch (command) {
            case "help":
                return  "Вот команды, которыми можно пользоваться:\n\n" +
                        ...
                        "add_min - добавить шляпу, если она меньше всех имеющихся\n" +
            ...
            case "add_min":
                return  "Эта команда идентична команде add (введите \"help add\", чтобы узнать о ней), но  \n"+
                        "в данном случае шляпа будет добавлена только в том случае, если она меньше уже имеющихся";
